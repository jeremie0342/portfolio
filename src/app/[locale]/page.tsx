...
export default async function Home({ params }: PageProps<"/[locale]">) {
  ...
  const [activity, totals, positions, credentials, profiles] =
    await Promise.all([
      readActivity(),
      readYearTotals(),
  ...
  return (
    <>
      {/* Outside main, and that is not a detail: while the opening plays, the
          stylesheet holds back every direct child of main, and a loader placed
          in there would hide itself.

          It lives on this page rather than in the layout because it introduces
          the wordmark, which is worth doing on arrival and not again on every
          page opened in a new tab. Elsewhere the attribute it sets is never
          written, so nothing holds those pages back at all. */}
      <Loader count={count} />
